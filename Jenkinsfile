...
node ('compile'){
    ...
    stage('Deploy') {
        ...
        build job: 'config_dhcp_tftp', parameters: [[$class: 'StringParameterValue', name: 'TREE_NAME', value: TREE_NAME]], wait: true

        if (! SKIP_UEFI) {
            build job: 'config_uefi', parameters: [[$class: 'StringParameterValue', name: 'TREE_NAME', value: TREE_NAME]], wait: true
        }

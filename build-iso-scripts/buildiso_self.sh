...
function scp_build_result() {
    ...
    if timeout 120 sshpass -p 'Master12#$' ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@${TARGET_IP} test -d "/mnt/data/fileserver_data/open-estuary/${target_dir}";then
        echo  "/mnt/data/fileserver_data/open-estuary/${target_dir} exist in ${TARGET_IP}"
    else
        sshpass -p 'Master12#$' scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -r /home/fileserver/open-estuary/* root@${TARGET_IP}:"/mnt/data/fileserver_data/open-estuary/self-test/" &
        sleep 3m
        wait
    fi
}
